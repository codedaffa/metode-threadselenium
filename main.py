...
@app.post("/selenium")
def input_seleniumhttpx(input: DataRequest):
    try:
        ...
        hasil = main(input.keyword, input.pages)
...
def data_product(soup_produk, product_link):
    driver = webdriver.Chrome("chromedriver.exe")
    try:
        print(f"Membuka page {product_link}...")
        driver.get(product_link)
        print(f"Menunggu reload page {product_link}...")
...
def data_shop(shop_link):
    driver = webdriver.Chrome("chromedriver.exe")
    try:
        print(f"Membuka page {shop_link}...")
        driver.get(shop_link)
        print(f"Menunggu reload page {shop_link}...")
        # WebDriverWait(driver, timeout=10).until(EC.presence_of_element_located((By.TAG_NAME, "body")))
        content = driver.page_source
        soup = BeautifulSoup(content, "html.parser")
